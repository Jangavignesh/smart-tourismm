...
const createGroupTrip = async (req, res) => {
  try {
    ...
    if (invitedEmails && invitedEmails.length > 0) {
      const invitedUsers = await User.find({ email: { $in: invitedEmails.map(e => e.toLowerCase().trim()) } });
      for (const invitedUser of invitedUsers) {
        if (invitedUser._id.toString() === req.user._id.toString()) continue;
        ...
        await Notification.create({
          ...
          type: "trip_invite",
          title: "✉️ New Trip Invitation!",
          message: `${req.user.name} invited you to join "${name}"`,
          link: `/group-trips/${trip._id}`,
          tripId: trip._id,
...
const inviteByEmail = async (req, res) => {
  try {
    ...
    await Notification.create({
      ...
      message: `${req.user.name} added you to "${trip.name}"`,
      link: `/group-trips/${trip._id}`,
      tripId: trip._id,
      triggeredBy: req.user.name,
    });
