...
const ItineraryPage = () => {
  ...
  const generateItinerary = async () => {
    ...
    try {
      ...
      // Slow networks + LLM latency: give it more time.
      const res = await api.post("/itinerary/generate-llm", payload, { timeout: 160000 });
      ...
      const debugSample = err?.response?.data?.debug?.sample;
      if (debugSample) {
        console.warn("Gemini invalid JSON sample:", debugSample);
      }
      const msg =
        status === 401 ? "Please log in again."
          : status === 501 ? "LLM itinerary isn't configured on the server."
            : status === 422 ? "Gemini returned an invalid response. Please try again."
              : status === 504 || err?.code === "ECONNABORTED" ? "Gemini is taking too long and the request timed out. Please try again."
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white">
        <div className="max-w-4xl mx-auto px-4 py-10">
          <p className="text-blue-200 text-sm font-medium mb-1">✨ Powered by AI</p>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-800 mb-2">AI is planning your trip...</h2>
